...
namespace irr
{
	namespace scene
	{
		...
		*/
		class ISceneNode : virtual public io::SIAttributeExchangingObject
		{
		public:
			...
			*/
			...
			virtual std::shared_ptr<ISceneManager> getSceneManager(void) const { return SceneManager.lock(); }
		...
		public:
			...
			void cloneMembers(std::shared_ptr<ISceneNode> toCopyFrom, std::shared_ptr<ISceneManager> newManager)
			{
				...
				{
					ISceneNodeAnimator* anim = (*ait)->createClone(std::dynamic_pointer_cast<ISceneNode>(shared_from_this()), SceneManager.lock());
					if (anim)
					{
						addAnimator(anim);
						anim->drop();
