...
namespace scene
{
	...
	*/
	...
	public:
		...
		virtual std::shared_ptr<ISceneNode> addSceneNode(ESCENE_NODE_TYPE type, std::shared_ptr<ISceneNode> parent = 0) = 0;
		...
		\return Returns pointer to the new scene node or null if not successful.
		This pointer should not be dropped. See IReferenceCounted::drop() for more information. */
		virtual std::shared_ptr<ISceneNode> addSceneNode(const c8* typeName,std::shared_ptr<ISceneNode> parent=0) = 0;
