...
namespace irr
{
	...
	//		M[14] = (T)(2.0f*zNear*zFar/(zNear-zFar)); // OpenGL version
		...
		*/
		...
		{
			__m128 tempX = _mm_set1_ps(vect.X);
			__m128 tempY = _mm_set1_ps(vect.Y);
			__m128 tempZ = _mm_set1_ps(vect.Z);
			...
			vect.Y = tmp.X*M[1] + tmp.Y*M[5] + tmp.Z*M[9];
			vect.Z = tmp.X*M[2] + tmp.Y*M[6] + tmp.Z*M[10];*/
			_MM_EXTRACT_FLOAT(vect.X, result, 0);
			_MM_EXTRACT_FLOAT(vect.Y, result, 1);
			_MM_EXTRACT_FLOAT(vect.Z, result, 2);
		}
		template <>
		inline void CMatrix4<f32>::rotateVect(core::vector3df& out, const core::vector3df& in) const
		{
			__m128 tempX = _mm_load1_ps(&in.X);
			__m128 tempY = _mm_load1_ps(&in.Y);
			__m128 tempZ = _mm_load1_ps(&in.Z);
		...
		//! An alternate transform vector method, writing into an array of 3 floats
		template <>
		inline void CMatrix4<f32>::rotateVect(f32* out, const core::vector3df& in) const
		{
			__m128 tempX = _mm_load1_ps(&in.X);
			__m128 tempY = _mm_load1_ps(&in.Y);
			__m128 tempZ = _mm_load1_ps(&in.Z);
			...
			_MM_EXTRACT_FLOAT(out[0], result, 0);
			_MM_EXTRACT_FLOAT(out[1], result, 1);
			_MM_EXTRACT_FLOAT(out[2], result, 2);
		}

		template <>
		inline void CMatrix4<f32>::inverseRotateVect(vector3df& vect) const
		{
			__m128 tempX = _mm_set_ps(0, vect.Z, vect.Y, vect.X);

			__m128 M0 = _mm_load_ps(&M[0]);
			__m128 M4 = _mm_load_ps(&M[4]);
			__m128 M8 = _mm_load_ps(&M[8]);

			__m128 R0 = _mm_mul_ps(tempX, M0);
			__m128 R1 = _mm_mul_ps(tempX, M4);
			__m128 R2 = _mm_mul_ps(tempX, M8);

			R0 = _mm_hadd_ps(R0, R0);
			R0 = _mm_hadd_ps(R0, R0);

			_MM_EXTRACT_FLOAT(vect.X, R0, 3);// .m128_f32[0];

			R1 = _mm_hadd_ps(R1, R1);
			R1 = _mm_hadd_ps(R1, R1);

			_MM_EXTRACT_FLOAT(vect.Y, R1, 3);// .m128_f32[0];

			R2 = _mm_hadd_ps(R2, R2);
			R2 = _mm_hadd_ps(R2, R2);

			_MM_EXTRACT_FLOAT(vect.Z, R2, 3);// .m128_f32[0];

			//vect.X = R0.m128_f32[0] + R0.m128_f32[1] + R0.m128_f32[2];
			//vect.Y = R1.m128_f32[0]+R1.m128_f32[1]+R1.m128_f32[2];
			//vect.Z = R2.m128_f32[0]+R2.m128_f32[1]+R2.m128_f32[2];
			/*
			vector3df tmp = vect;
			vect.X = tmp.X*M[0] + tmp.Y*M[1] + tmp.Z*M[2];
			vect.Y = tmp.X*M[4] + tmp.Y*M[5] + tmp.Z*M[6];
		...
		template <>
		inline void CMatrix4<f32>::transformVect(vector3df& vect) const
		{
			...
			__m128 M4 = _mm_load_ps(&M[4]);
			__m128 M8 = _mm_load_ps(&M[8]);
			__m128 M12 = _mm_load_ps(&M[12]);

			__m128 X = _mm_load1_ps(&vect.X);
			__m128 Y = _mm_load1_ps(&vect.Y);
			__m128 Z = _mm_load1_ps(&vect.Z);
			_mm_store_ps(vector,
				_mm_add_ps(
					_mm_add_ps(
						_mm_add_ps(
		...
		template <>
		inline void CMatrix4<f32>::transformVect(vector3df& out, const vector3df& in) const
		{
			__m128 M0 = _mm_load_ps(&M[0]);
			__m128 M4 = _mm_load_ps(&M[4]);
			__m128 M8 = _mm_load_ps(&M[8]);
			...
			__m128 X = _mm_load1_ps(&in.X);
			__m128 Y = _mm_load1_ps(&in.Y);
			__m128 Z = _mm_load1_ps(&in.Z);
		...
		template <>
		inline void CMatrix4<f32>::transformVect(f32* out, const core::vector3df& in) const
		{
			__m128 M0 = _mm_load_ps(&M[0]);
			__m128 M4 = _mm_load_ps(&M[4]);
			__m128 M8 = _mm_load_ps(&M[8]);
		...
		template <>
		inline void CMatrix4<f32>::transformVec3(f32* out, const f32* in) const
		{
			__m128 M0 = _mm_load_ps(&M[0]);
			__m128 M4 = _mm_load_ps(&M[4]);
			__m128 M8 = _mm_load_ps(&M[8]);
		...
		template <>
		inline CMatrix4<f32> CMatrix4<f32>::operator+(const CMatrix4<f32>& other) const
		{
			CMatrix4<f32> temp(EM4CONST_NOTHING);

			_mm_store_ps(&temp[0], _mm_add_ps(_mm_load_ps(&M[0]), _mm_load_ps(&other[0])));
			_mm_store_ps(&temp[4], _mm_add_ps(_mm_load_ps(&M[4]), _mm_load_ps(&other[4])));
			_mm_store_ps(&temp[8], _mm_add_ps(_mm_load_ps(&M[8]), _mm_load_ps(&other[8])));
			_mm_store_ps(&temp[12], _mm_add_ps(_mm_load_ps(&M[12]), _mm_load_ps(&other[12])));
		...
		//! Add another matrix.
		template <>
		inline CMatrix4<f32>& CMatrix4<f32>::operator+=(const CMatrix4<f32>& other)
		{
			_mm_store_ps(&M[0], _mm_add_ps(_mm_load_ps(&M[0]), _mm_load_ps(&other[0])));
			_mm_store_ps(&M[4], _mm_add_ps(_mm_load_ps(&M[4]), _mm_load_ps(&other[4])));
			_mm_store_ps(&M[8], _mm_add_ps(_mm_load_ps(&M[8]), _mm_load_ps(&other[8])));
			_mm_store_ps(&M[12], _mm_add_ps(_mm_load_ps(&M[12]), _mm_load_ps(&other[12])));
		...
		inline CMatrix4<f32> CMatrix4<f32>::operator-(const CMatrix4<f32>& other) const
		{
			...
			_mm_store_ps(&temp[0], _mm_sub_ps(_mm_load_ps(&M[0]), _mm_load_ps(&other[0])));
			_mm_store_ps(&temp[4], _mm_sub_ps(_mm_load_ps(&M[4]), _mm_load_ps(&other[4])));
			_mm_store_ps(&temp[8], _mm_sub_ps(_mm_load_ps(&M[8]), _mm_load_ps(&other[8])));
			_mm_store_ps(&temp[12], _mm_sub_ps(_mm_load_ps(&M[12]), _mm_load_ps(&other[12])));
		...
		inline CMatrix4<f32>& CMatrix4<f32>::operator-=(const CMatrix4<f32>& other)
		{
			_mm_store_ps(&M[0], _mm_sub_ps(_mm_load_ps(&M[0]), _mm_load_ps(&other[0])));
			_mm_store_ps(&M[4], _mm_sub_ps(_mm_load_ps(&M[4]), _mm_load_ps(&other[4])));
			_mm_store_ps(&M[8], _mm_sub_ps(_mm_load_ps(&M[8]), _mm_load_ps(&other[8])));
			_mm_store_ps(&M[12], _mm_sub_ps(_mm_load_ps(&M[12]), _mm_load_ps(&other[12])));
		...
		//! Multiply by scalar.
		template <>
		inline CMatrix4<f32> CMatrix4<f32>::operator*(const f32& scalar) const
		{
			CMatrix4<f32> temp(EM4CONST_NOTHING);

			_mm_store_ps(&temp[0], _mm_mul_ps(_mm_load_ps(&M[0]), _mm_load1_ps(&scalar)));
			_mm_store_ps(&temp[4], _mm_mul_ps(_mm_load_ps(&M[4]), _mm_load1_ps(&scalar)));
			_mm_store_ps(&temp[8], _mm_mul_ps(_mm_load_ps(&M[8]), _mm_load1_ps(&scalar)));
			_mm_store_ps(&temp[12], _mm_mul_ps(_mm_load_ps(&M[12]), _mm_load1_ps(&scalar)));
		...
		//! Multiply by scalar.
		template <>
		inline CMatrix4<f32>& CMatrix4<f32>::operator*=(const f32& scalar)
		{
			_mm_store_ps(&M[0], _mm_mul_ps(_mm_load_ps(&M[0]), _mm_load1_ps(&scalar)));
			_mm_store_ps(&M[4], _mm_mul_ps(_mm_load_ps(&M[4]), _mm_load1_ps(&scalar)));
			_mm_store_ps(&M[8], _mm_mul_ps(_mm_load_ps(&M[8]), _mm_load1_ps(&scalar)));
