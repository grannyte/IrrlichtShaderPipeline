...
namespace irr
{
	namespace video
	{
		...
		public:
			...
			// Size of this context's own render target (see getRenderTarget()), (0,0) before one
			// has been created.
			virtual core::dimension2d<u32> getRecordingSize() const = 0;

			// The texture this context draws into instead of the immediate driver's back buffer,
			// to be composed into a later frame like any other texture.
