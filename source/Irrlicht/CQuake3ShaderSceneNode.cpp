	...
	namespace scene
	{
		...
		*/
			...
			Shader(shader), Mesh(0), Shadow(0), Original(0), MeshBuffer(0), TimeAbs(0.f)
		{
			...
			MeshBuffer = new CMeshBuffer<video::S3DVertex>(SceneManager.lock()->getVideoDriver()->getVertexDescriptor(0));
		...
		*/
		void CQuake3ShaderSceneNode::cloneBuffer(scene::CMeshBuffer<video::S3DVertex>* dest, const scene::CMeshBuffer<video::S3DVertex2TCoords>* buffer, bool translateCenter)
		{
			...
			if (translateCenter)
			{
				...
				SceneManager.lock()->getMeshManipulator()->transform(dest, m);
			}
		...
		*/
		void CQuake3ShaderSceneNode::OnRegisterSceneNode()
		{
			if (isVisible())
			{
				SceneManager.lock()->registerNodeForRendering(std::dynamic_pointer_cast<ISceneNode>(shared_from_this()), getRenderStage());
			}
		...
		*/
		...
		{
			video::IVideoDriver* driver = SceneManager.lock()->getVideoDriver();
			E_SCENE_NODE_RENDER_PASS pass = SceneManager.lock()->getSceneNodeRenderPass();
			...
			if (group->isDefined("polygonoffset"))
			{
				...
				/*
						f32 n = SceneManager.lock()->getActiveCamera()->getNearValue();
						f32 f = SceneManager.lock()->getActiveCamera()->getFarValue ();
			...
			if (DebugDataVisible & scene::EDS_NORMALS)
			{
				...
				IAnimatedMesh* arrow = SceneManager.lock()->addArrowMesh(
					"__debugnormalq3",
				...
				if (0 == arrow)
				{
					arrow = SceneManager.lock()->getMesh("__debugnormalq3");
				}
		...
		std::shared_ptr<IShadowVolumeSceneNode> CQuake3ShaderSceneNode::addShadowVolumeSceneNode(
		...
		*/
		void CQuake3ShaderSceneNode::deformvertexes_autosprite(f32 dt, SModifierFunction& function)
		{
			...
			const core::vector3df& camPos = SceneManager.lock()->getActiveCamera()->getPosition();
		...
		*/
		void CQuake3ShaderSceneNode::deformvertexes_autosprite2(f32 dt, SModifierFunction& function)
		{
			...
			const core::vector3df camPos = SceneManager.lock()->getActiveCamera()->getAbsolutePosition();
		...
		*/
		void CQuake3ShaderSceneNode::vertextransform_alphagen(f32 dt, SModifierFunction& function)
		{
			...
			case LIGHTINGSPECULAR:
			{
				...
				const SViewFrustum* frustum = SceneManager.lock()->getActiveCamera()->getViewFrustum();
