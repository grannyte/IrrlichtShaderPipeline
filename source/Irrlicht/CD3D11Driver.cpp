	...
	namespace video
	{
		...
		inline CD3D11HardwareBuffer* CD3D11Driver::GetTempBuffer(E_HARDWARE_BUFFER_TYPE type, irr::u32 size, irr::u32 flags, const void* initialData)
		{
			if (!MeshBuffer2dQueue.empty())
			{
				...
				mb2d->update(irr::scene::E_HARDWARE_MAPPING::EHM_DYNAMIC, size, initialData);
		...
		bool CD3D11Driver::initDriver(HWND hwnd, bool pureSoftware)
		{
			...
			EvaluateBestDepthFormat();

			// creating depth buffer
			DefaultDepthBuffer = createDepthStencilView(ScreenSize, getColorFormatFromD3DFormat(DepthStencilFormat));
		...
		bool CD3D11Driver::setActiveTexture(u32 stage, video::ITexture* texture)
		{
			...
			CurrentTexture[stage] = (texture == NULL && stage == 0) ? NullTexture : texture;
		...
		{
			//os::Printer::log((irr::core::stringc(" set material id: ") += (int)material.MaterialType).c_str(), ELL_ERROR);
			Material = material;
		...
		void CD3D11Driver::drawMeshBuffer(const scene::IMeshBuffer* mb)
		{
			...
			for (u32 i = 0; i < mb->getVertexBufferCount(); ++i)
			{
				...
				else if (mb->getVertexDescriptor()->getInstanceDataStepRate(i) > EIDSR_PER_INSTANCE)
				{
					instanceVertexCount *= mb->getVertexBuffer(i)->getVertexCount();
				}
			}
			...
			bool clearZBuffer, SColor color, video::ITexture* depthStencil)
		{
			...
			if (depthStencil && !depthStencil->isDeptStencil())
			{
				os::Printer::log("Fatal Error: Tried to set a non depthstencil texture as depth texture.", ELL_ERROR);
				return false;
			}

			CD3D11Texture* tex = static_cast<CD3D11Texture*>(texture);
			CD3D11Texture* depth = static_cast<CD3D11Texture*>(depthStencil);

			if (depthStencil)
				CurrentDepthBuffer = (CD3D11Texture*)depthStencil;
			else if (texture)
				CurrentDepthBuffer = checkDepthBuffer(tex);
			else
				CurrentDepthBuffer = DefaultDepthBuffer;

			ID3D11DepthStencilView* dsview = NULL;
			if (CurrentDepthBuffer && ((!tex) || tex->getTextureType() != ETT_2D_ARRAY))
				dsview = CurrentDepthBuffer->dsView;
			...
			if (tex == NULL)
			{
				...
				//os::Printer::log("DefaultDepthBufferSet", irr::ELL_INFORMATION);
				Context->OMSetRenderTargets(1, &CurrentBackBuffer, dsview);
			...
			// clear views
			if (clearBackBuffer && CurrentBackBuffer)
			{
				SColorf fCol(color);
		...
		bool CD3D11Driver::setRenderTarget(const core::array<video::IRenderTarget>& targets,
			const core::array<bool>& clearBackBuffer, bool clearZBuffer, SColor color, video::ITexture* depthStencil)
		{
			u32 i;
			...
			if (!targets.size())
			{
				...
				return setRenderTarget(NULL, 0, clearZBuffer, color, depthStencil);
			}
			...
			for (i = 0; i < maxMultipleRTTs; ++i)
			{
				...
				if (clearBackBuffer[min(i, clearBackBuffer.size() - 1)] && RTViews[i])
			...
			// set targets
			Context->OMSetRenderTargets(maxMultipleRTTs, RTViews, CurrentDepthBuffer->dsView);
		...
		video::ITexture* CD3D11Driver::createDeviceDependentTexture(const core::array<ITexture*>& surfaces, const E_TEXTURE_TYPE Type, const io::path& name, void* mipmapData)
		{
			return new CD3D11Texture(&surfaces, this, TextureCreationFlags, name, Type, surfaces.size(), mipmapData);
		}
		void CD3D11Driver::renderArray(u32 vertexCount, u32 indexCount, scene::E_PRIMITIVE_TYPE pType, u32 numInstances)
		{
			// Bind depth-stencil view
			BridgeCalls->setDepthStencilState(DepthStencilDesc);
			...
			video::SColor leftDownEdge, video::SColor rightDownEdge)
		{
			...
			{
				Context->ClearDepthStencilView(DefaultDepthBuffer->dsView, D3D11_CLEAR_STENCIL, 0.0f, 0);
			}
		}
		...
		//! sets the needed renderstates
		bool CD3D11Driver::setRenderStates3DMode(CD3D11VertexDescriptor* vType)
		{
			if (!Device)
				return false;
		...
		void CD3D11Driver::setRenderStates2DMode(bool alpha, bool texture, bool alphaChannel)
		{
			...
			//if (CurrentRenderMode != ERM_2D || Transformation3DChanged)
			{
				...
				if (!OverrideMaterial2DEnabled)
				{
					...
					OverrideMaterial2DEnabled = true;
				}
		...
		void CD3D11Driver::clearZBuffer()
		{
			if (CurrentDepthBuffer)
				Context->ClearDepthStencilView(CurrentDepthBuffer->dsView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 0.0f, 0);
		}
		...
		DXGI_FORMAT CD3D11Driver::getD3DFormatFromColorFormat(ECOLOR_FORMAT format) const
		{
			...
			default:
				std::cout << "Color format : " << format << "Unsupported in irrlicht dx11" << std::endl;
				return DXGI_FORMAT_UNKNOWN;
			}
		}
		...
		ECOLOR_FORMAT CD3D11Driver::getColorFormatFromD3DFormat(DXGI_FORMAT format) const
		{
			...
			case DXGI_FORMAT_R32G32B32_FLOAT:
				return ECF_B32G32R32F;
			case DXGI_FORMAT_R32G32B32A32_FLOAT:
				return ECF_A32B32G32R32F;
			case DXGI_FORMAT_R8G8B8A8_SNORM:
				return ECF_A8R8G8B8S;
		...
		CD3D11Texture* CD3D11Driver::checkDepthBuffer(ITexture* tex)
		{
			if (!tex)
				return 0;
		...
		void CD3D11Driver::removeDepthSurface(CD3D11Texture* depth)
		{
		...
		void CD3D11Driver::reset()
		{
...
#ifdef _DEBUG
			hr = SwapChain->ResizeBuffers(2, ScreenSize.Width, ScreenSize.Height, D3DColorFormat, DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH);
			if (FAILED(hr))
			{
			...
			for (i = 0; i < size; ++i)
			{
				auto ldep = DepthBuffers[i];
				ldep->drop();
				DepthBuffers[i] = createDepthStencilView(ldep->Size, ldep->ColorFormat);
			}
			// Set render targets
			Context->OMSetRenderTargets(1, &DefaultBackBuffer, DefaultDepthBuffer->dsView);
		...
		s32 CD3D11Driver::addHighLevelShaderMaterial(
			...
			E_MATERIAL_TYPE baseRenderer, IVertexDescriptor* vertexTypeOut, s32 userData, E_GPU_SHADING_LANGUAGE shadingLang)
		{
			s32 id = -1;
			CD3D11MaterialRenderer* rend =
				new CD3D11MaterialRenderer(Device, this, BridgeCalls, id,
					...
					inType, outType, verticesOut, (CD3D11VertexDescriptor*)vertexTypeOut,
					callback, getMaterialRenderer(baseRenderer), userData, FileSystem, shadingLang);
			rend->drop();
		...
		void CD3D11Driver::setGeometryShaderConstant(const f32* data, s32 startRegister, s32 constantAmount)
		{
			os::Printer::log("\"setGeometryShaderConstant\" with offset is not supported.", ELL_ERROR);
		}

		void CD3D11Driver::setHullShaderConstant(const f32* data, s32 startRegister, s32 constantAmount)
		{
			os::Printer::log("\"setHullShaderConstant\" with offset is not supported.", ELL_ERROR);
		}
		void CD3D11Driver::setDomainShaderConstant(const f32* data, s32 startRegister, s32 constantAmount)
		{
			os::Printer::log("\"setDomainShaderConstant\" with offset is not supported.", ELL_ERROR);
		}
		...
		bool CD3D11Driver::setStreamOutputBuffer(scene::IVertexBuffer* buffer)
		{
			...
			// Validate buffer
			if (hwBuff->getDriverType() != EDT_DIRECT3D11)
			{
