...
namespace irr
{
	namespace scene
	{
		...
		std::shared_ptr<ISceneNode> CSceneCollisionManager::getSceneNodeFromScreenCoordinatesBB(
			const core::position2d<s32>& pos, s32 idBitMask, bool noDebugObjects, std::shared_ptr<ISceneNode> root)
		{
		...
		std::shared_ptr<ISceneNode> CSceneCollisionManager::getSceneNodeFromRayBB(
			const core::line3d<f32>& ray,
			s32 idBitMask, bool noDebugObjects, std::shared_ptr<ISceneNode> root)
		{
			std::shared_ptr<ISceneNode> best = 0;
			f32 dist = FLT_MAX;
		...
		void CSceneCollisionManager::getPickedNodeBB(std::shared_ptr<ISceneNode> root,
			core::line3df& ray, s32 bits, bool noDebugObjects,
			f32& outbestdistance, std::shared_ptr<ISceneNode>& outbestnode)
		{
		...
		std::shared_ptr<ISceneNode> CSceneCollisionManager::getSceneNodeAndCollisionPointFromRay(
			const core::line3df& ray,
			core::vector3df& outCollisionPoint,
			core::triangle3df& outTriangle,
			s32 idBitMask,
			std::shared_ptr<ISceneNode> collisionRootNode,
			bool noDebugObjects)
		{
			std::shared_ptr<ISceneNode> bestNode = 0;
		...
		void CSceneCollisionManager::getPickedNodeFromBBAndSelector(
			std::shared_ptr<ISceneNode> root,
			core::line3df& ray,
			s32 bits,
			bool noDebugObjects,
			f32& outBestDistanceSquared,
			std::shared_ptr<ISceneNode>& outBestNode,
			...
			core::triangle3df& outBestTriangle)
		{
			...
			for (; it != children.end(); ++it)
			{
					...
					(bits == 0 || (bits != 0 && (current->getID() & bits))))
				{
					...
					std::shared_ptr<ISceneNode> hitNode = 0;
		...
		std::shared_ptr<ISceneNode> CSceneCollisionManager::getSceneNodeFromCameraBB(
			std::shared_ptr<ICameraSceneNode> camera, s32 idBitMask, bool noDebugObjects)
		{
			if (!camera)
				return 0;
		...
		core::line3d<f32> CSceneCollisionManager::getRayFromScreenCoordinates(
			const core::position2d<s32>& pos, std::shared_ptr<ICameraSceneNode> camera)
		{
		...
		core::position2d<s32> CSceneCollisionManager::getScreenCoordinatesFrom3DPosition(
			const core::vector3df& pos3d, std::shared_ptr<ICameraSceneNode> camera, bool useViewPort)
		{
