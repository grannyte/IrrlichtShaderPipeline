...
void COpenGLTextureArray::uploadTexture(const core::array<ITexture*> &surfaces)
{
	...
	// Define it
	Driver->glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, InternalFormat, w, h, nfiles, 0,
			PixelFormat, PixelType, NULL);

	for (i = 0; i < nfiles; i++)
	{
		void *src = surfaces[i]->lock(ETLM_READ_WRITE);

		Driver->setActiveTexture(0, this);
		if (Driver->testGLError())
			os::Printer::log("Could not bind Texture", ELL_ERROR);

		Driver->glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, w, h, 1,
