...
namespace video
{
	...
	E_INSTANCE_DATA_STEP_RATE CVertexDescriptor::getInstanceDataStepRate(u32 bufferID) const
	{
		...
		if (bufferID < EInstanceDataStepRate.size())
			Rate = EInstanceDataStepRate[bufferID];

		return Rate;
	}

	void CVertexDescriptor::setInstanceDataStepRate(E_INSTANCE_DATA_STEP_RATE rate, u32 bufferID, u32 IRate)
	{
		if (bufferID < EInstanceDataStepRate.size())
		{
			EInstanceDataStepRate[bufferID] = rate;
			InstanceDataStepRate[bufferID] = IRate;
		}
	}
	...
	bool CVertexDescriptor::addAttribute(const core::stringc& name, u32 elementCount, E_VERTEX_ATTRIBUTE_SEMANTIC semantic, E_VERTEX_ATTRIBUTE_TYPE type, u32 bufferID)
	{
		...
		for (u32 i = EInstanceDataStepRate.size(); i <= bufferID; ++i)
		{
			...
			InstanceDataStepRate.push_back(0);
		}
