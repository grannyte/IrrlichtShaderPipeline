...
namespace irr
{
	namespace video
	{
		...
		core::array<D3D11_SO_DECLARATION_ENTRY>& CD3D11VertexDescriptor::getOutputLayoutDescription()
		{
			if (OutputLayoutDesc.empty())
				rebuildOutput();

			return OutputLayoutDesc;
		}
		...
		void CD3D11VertexDescriptor::rebuild()
		{
			...
			for (u32 i = 0; i < size; ++i)
			{
				...
				if (getInstanceDataStepRate(Attribute[i]->getBufferID()) == EIDSR_PER_VERTEX)
				{
		...
		void CD3D11VertexDescriptor::rebuildOutput()
		{
			// if don't exists, create layout
