...
namespace irr
{
	namespace scene
	{
		...
		std::shared_ptr<ISceneNode> CDefaultSceneNodeFactory::addSceneNode(ESCENE_NODE_TYPE type, std::shared_ptr<ISceneNode> parent)
		{
			switch (type)
			{
			case ESNT_CUBE:
			...
			{
			//	auto val = Manager->addVolumeLightSceneNode(parent);
		//	return std::dynamic_pointer_cast<ISceneNode>(val);
			}
			default:
				break;
			}
		...
		std::shared_ptr<ISceneNode> CDefaultSceneNodeFactory::addSceneNode(const c8* typeName,std::shared_ptr<ISceneNode> parent)
		{
