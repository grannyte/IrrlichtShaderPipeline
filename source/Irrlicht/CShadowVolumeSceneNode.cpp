...
namespace irr
{
	namespace scene
	{
		...
		//! constructor
		CShadowVolumeSceneNode::CShadowVolumeSceneNode(const IMesh* shadowMesh,
			std::shared_ptr<ISceneManager> mgr, s32 id, bool zfailmethod, f32 infinity)
			: IShadowVolumeSceneNode(mgr, id),
...
#define IRR_USE_REVERSE_EXTRUDED
		...
		void CShadowVolumeSceneNode::updateShadowVolumes()
		{
			...
			const u32 lightCount = SceneManager.lock()->getVideoDriver()->getDynamicLightCount();
			if (!lightCount)
				return;
			...
			{
				const video::SLight& dl = SceneManager.lock()->getVideoDriver()->getDynamicLight(i);
		...
		void CShadowVolumeSceneNode::OnRegisterSceneNode()
		{
			if (IsVisible)
			{
				SceneManager.lock()->registerNodeForRendering(std::dynamic_pointer_cast<ISceneNode>(shared_from_this()), scene::ESNRP_SHADOW);
		...
		void CShadowVolumeSceneNode::render()
		{
			...
			for (u32 i = 0; i < ShadowVolumesUsed; ++i)
			{
				...
				if (UseZFailMethod && SceneManager.lock()->getActiveCamera())
				{
					...
					f32 maxDistance = core::vector3df(SceneManager.lock()->getActiveCamera()->getPosition() - edges[0]).getLength();
					f32 curDistance = 0.f;

					for (int j = 1; j < 8; ++j)
					{
						curDistance = core::vector3df(SceneManager.lock()->getActiveCamera()->getPosition() - edges[j]).getLength();
