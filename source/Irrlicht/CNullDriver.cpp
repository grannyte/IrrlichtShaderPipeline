...
namespace irr
{
	namespace video
	{
		...
		//! THIS METHOD HAS TO BE OVERRIDDEN BY DERIVED DRIVERS WITH OWN TEXTURES
		ITexture* CNullDriver::createDeviceDependentTexture(const core::array<ITexture*>& surfaces, const E_TEXTURE_TYPE Type, const io::path& name, void* mipmapData)
		{
			return new SDummyTexture(name);
		}
		...
		bool CNullDriver::setStreamOutputBuffer(scene::IVertexBuffer* buffer)
		{
			os::Printer::log("Stream Output buffer not support in this driver.", ELL_WARNING);
			return false;
		}
		...
		bool CNullDriver::writeImageToFile(IImage* image, io::IWriteFile* file, u32 param)
		{
			...
			for (s32 i = SurfaceWriter.size() - 1; i >= 0; --i)
			{
				...
				{
					std::cout << "Suitable extention found " << std::endl;
		...
		void CNullDriver::drawMeshBuffer(const scene::IMeshBuffer* mb)
		{
			...
			if ((mb->getIndexBuffer()->getType() == EIT_16BIT) && (mb->getVertexBuffer(0)->getVertexCount() > 65536))
				os::Printer::log("Too many vertices for 16bit index type, render artifacts may occur.");
			irr::u32 instancecount = 1;
			for (int i = 0; i < mb->getVertexBufferCount(); ++i) {
				if (mb->getVertexDescriptor()->getInstanceDataStepRate(i) == EIDSR_PER_INSTANCE)
				{
					instancecount = mb->getVertexBuffer(i)->getVertexCount();
				}
			}

			PrimitivesDrawn += mb->getPrimitiveCount() * instancecount;
		...
		bool CNullDriver::isHardwareBufferRecommend(const scene::IMeshBuffer* mb)
		{
			if (!mb || (mb->getHardwareMappingHint_Index() == scene::EHM_NEVER && mb->getHardwareMappingHint_Vertex() == scene::EHM_NEVER))
			{
				...
				if (mb->getHardwareMappingHint_Index() == scene::EHM_NEVER)
					os::Printer::log("index flagged never");
				if (mb->getHardwareMappingHint_Vertex() == scene::EHM_NEVER)
					os::Printer::log("vertex flagged never");
#endif
				return false;
			}
			...
			{
#ifdef debug
				os::Printer::log("Buffer to small");
#endif
				return false;
			}
		...
		s32 CNullDriver::addHighLevelShaderMaterial(
			...
			const c8* hullShaderProgram,

			const c8* hullShaderEntryPointName,

			E_HULL_SHADER_TYPE hsCompileTarget,

			const c8* domainShaderProgram,

			const c8* domainShaderEntryPointName,
		...
		{
			os::Printer::log("Hull And domain Shader not yet availible sorry");
			return -1;
		}
			...
			s32 userData, E_GPU_SHADING_LANGUAGE shadingLang)
		{
			...
			s32 result = addHighLevelShaderMaterialFromFiles(
				...
				callback, baseMaterial, vertexTypeOut, userData, shadingLang);
		...
		s32 CNullDriver::addHighLevelShaderMaterialFromFiles(
			...
			const io::path& hullShaderProgramFileName,

			const c8* hullShaderEntryPointName,

			E_HULL_SHADER_TYPE hsCompileTarget,

			const io::path& domainShaderProgramFileName,
			...
			s32 userData, E_GPU_SHADING_LANGUAGE shadingLang)
		{
			...
			io::IReadFile* psfile = 0;
			io::IReadFile* gsfile = 0;
			io::IReadFile* hsfile = 0;
			io::IReadFile* dsfile = 0;
			...
			if (hullShaderProgramFileName.size())
			{
				hsfile = FileSystem->createAndOpenFile(hullShaderProgramFileName);
				if (!hsfile)
				{
					os::Printer::log("Could not open geometry shader program file",
						hullShaderProgramFileName, ELL_WARNING);
				}
			}
			if (domainShaderProgramFileName.size())
			{
				dsfile = FileSystem->createAndOpenFile(domainShaderProgramFileName);
				if (!dsfile)
				{
					os::Printer::log("Could not open geometry shader program file",
						domainShaderProgramFileName, ELL_WARNING);
				}
			}
			...
			s32 result = addHighLevelShaderMaterialFromFiles(
				...
				psfile, pixelShaderEntryPointName, psCompileTarget,
				gsfile, geometryShaderEntryPointName, gsCompileTarget,
				hsfile, hullShaderEntryPointName, hsCompileTarget,
				dsfile, domainShaderEntryPointName, dsCompileTarget,
				inType, outType, verticesOut,
				callback, baseMaterial, vertexTypeOut, userData, shadingLang);
		...
		s32 CNullDriver::addHighLevelShaderMaterialFromFiles(
			...
			io::IReadFile* hullShaderProgram,

			const c8* hullShaderEntryPointName,

			E_HULL_SHADER_TYPE hsCompileTarget,

			io::IReadFile* domainShaderProgram,
			...
			s32 userData, E_GPU_SHADING_LANGUAGE shadingLang)
		{
			...
			if (hullShaderProgram)
			{
				const long size = hullShaderProgram->getSize();
				if (size)
				{
					// if both handles are the same we must reset the file
					if ((hullShaderProgram == vertexShaderProgram) ||
						(hullShaderProgram == pixelShaderProgram) ||
						(hullShaderProgram == geometryShaderProgram))
						hullShaderProgram->seek(0);
					hs = new c8[size + 2];
					hullShaderProgram->read(hs, size);
					hs[size] = 0;
					hs[size + 1] = EOF;
				}
			}

			if (domainShaderProgram)
			{
				const long size = domainShaderProgram->getSize();
				if (size)
				{
					// if both handles are the same we must reset the file
					if ((domainShaderProgram == vertexShaderProgram) ||
						(domainShaderProgram == pixelShaderProgram) ||
						(domainShaderProgram == geometryShaderProgram) ||
						(domainShaderProgram == hullShaderProgram))
						domainShaderProgram->seek(0);
					ds = new c8[size + 2];
					domainShaderProgram->read(ds, size);
					ds[size] = 0;
					ds[size + 1] = EOF;
				}
			}
			...
			s32 result = this->addHighLevelShaderMaterial(
				...
				hs, hullShaderEntryPointName, hsCompileTarget,
				ds, domainShaderEntryPointName, dsCompileTarget,
				inType, outType, verticesOut,
				callback, baseMaterial, vertexTypeOut, userData, shadingLang);
			...
			E_MATERIAL_TYPE baseMaterial, IVertexDescriptor* vertexTypeOut, s32 userData, E_GPU_SHADING_LANGUAGE shadingLang)
		{
			...
			if (vertexShaderProgram)
			{
				...
				if (size)
				{
					...
					vs[size] = 0;
					vs[size + 1] = EOF;
					if (shadingLang == EGSL_PCMP)
					{
						//this is legit abuse of this parameter
						vertexShaderEntryPointName = (c8*)size;
					}
				}
			}
			...
			if (pixelShaderProgram)
			{
				...
				if (size)
				{
					...
					{
						//this is legit abuse of this parameter
						pixelShaderEntryPointName = (c8*)size;
					}
				}
			}
			...
			if (geometryShaderProgram)
			{
				...
				if (size)
				{
					...
					{
						//this is legit abuse of this parameter
						geometryShaderEntryPointName = (c8*)size;
					}
				}
			}
