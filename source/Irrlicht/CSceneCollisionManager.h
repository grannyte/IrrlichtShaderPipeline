	...
	namespace scene
	{
		...
		public:
			...
			virtual std::shared_ptr<ISceneNode> getSceneNodeFromScreenCoordinatesBB(const core::position2d<s32>& pos,
				s32 idBitMask = 0, bool bNoDebugObjects = false, std::shared_ptr<ISceneNode> root = 0) _IRR_OVERRIDE_;
			...
			virtual std::shared_ptr<ISceneNode> getSceneNodeFromRayBB(const core::line3d<f32>& ray,
				...
				std::shared_ptr<ISceneNode> root = 0) _IRR_OVERRIDE_;
			...
			virtual std::shared_ptr<ISceneNode> getSceneNodeFromCameraBB(std::shared_ptr<ICameraSceneNode> camera,
				s32 idBitMask = 0,
				bool bNoDebugObjects = false) _IRR_OVERRIDE_;

			//! Finds the collision point of a line and lots of triangles, if there is one.
			virtual bool getCollisionPoint(const core::line3d<f32>& ray,
				ITriangleSelector* selector, core::vector3df& outCollisionPoint,
				core::triangle3df& outTriangle,
				std::shared_ptr<ISceneNode>& outNode) _IRR_OVERRIDE_;
			...
			virtual core::line3d<f32> getRayFromScreenCoordinates(
				const core::position2d<s32>& pos, std::shared_ptr<ICameraSceneNode> camera = 0) _IRR_OVERRIDE_;

			//! Calculates 2d screen position from a 3d position.
			virtual core::position2d<s32> getScreenCoordinatesFrom3DPosition(
				const core::vector3df& pos, std::shared_ptr<ICameraSceneNode> camera = 0, bool useViewPort = false) _IRR_OVERRIDE_;
		...
		private:
			...
			void getPickedNodeBB(std::shared_ptr<ISceneNode> root, core::line3df& ray, s32 bits,
				bool bNoDebugObjects,
				f32& outbestdistance, std::shared_ptr<ISceneNode>& outbestnode);

			//! recursive method for going through all scene nodes
			void getPickedNodeFromBBAndSelector(std::shared_ptr<ISceneNode> root,
				core::line3df& ray,
				s32 bits,
				bool noDebugObjects,
